from adventofcode.problems.days.day208.internal.inputtextparser import InputTextParser
...
def test_instruction_type_dec():
    parsed_input = InputTextParser().parse("reg dec 5 if cond > 1")
    assert InstructionType.DEC == parsed_input.instructions[0]._instruction_type


def test_value_to_apply():
    parsed_input = InputTextParser().parse("reg inc 10001 if cond > 1")
...
def test_condition_value():
    parsed_input = InputTextParser().parse("reg inc 5 if cond != 98765")
